...
def main():
    ...
    axes[0].plot(merged["t"], merged["gt_speed"], marker=".", linestyle="-", color="#000000", label="true (ground truth)", markersize=4, alpha=0.7)
    axes[0].plot(merged["t"], merged["kf_speed"], marker=".", linestyle="-", color="#2E7D32", label="KF-smoothed estimate", markersize=4, alpha=0.7)
    axes[0].set_xlabel("sim time [s]")
    axes[0].set_ylabel("speed [m/s]")
    axes[0].set_title("Speed over time")
    axes[0].legend()

    axes[1].plot(merged["gt_x"], merged["gt_y"], linestyle="--", color="#000000", label="ground truth", linewidth=1.5, alpha=0.6)
    axes[1].plot(merged["x"], merged["y"], marker=".", linestyle="-", color="#2E7D32", label="KF filtered position", markersize=4)
